// Android bridge (pure C++; matches your iOS .mm logic)
// Build into a shared lib (.so) alongside Stockfish sources.
...
// Stockfish headers (path assumes this file sits in stockfish/src)
#include "bitboard.h"
#include "position.h"
#include "search.h"
#include "thread.h"
...
protected:
    int_type underflow() override {
        ...
        curr_ = std::move(q_.in_lines.front());
        q_.in_lines.pop();
        lk.unlock();
...
protected:
    ...
    int_type overflow(int_type ch) override {
        ...
        if (c == '\n') {
            ...
            // safety: avoid unbounded growth if engine outputs no '\n'
...
struct Engine {
    ...
    void start() {
        ...
        t = std::thread([this] {
            // ---- iOS had @autoreleasepool here; not needed on Android ----
            Trace(q, "thread start");

            // Stockfish global init (keep same order/behavior)
    ...
    const char* poll() {
        ...
        last = std::move(q.out_lines.front());
        q.out_lines.pop();
        return last.c_str();
    }
};
...
// ---- C API (exported) ----
extern "C" __attribute__((visibility("default"))) void _stockfish_start() {
...
extern "C" __attribute__((visibility("default"))) void _stockfish_stop() {
    using namespace StockfishBridge;
    if (!engine) return;
    engine->stop();
    delete engine;
...
extern "C" __attribute__((visibility("default"))) void _stockfish_send(const char* cmd) {
    using namespace StockfishBridge;
    if (!engine) return;
    engine->send(cmd);
}

extern "C" __attribute__((visibility("default"))) int _stockfish_read_copy(char* dst, int maxLen) {
    using namespace StockfishBridge;
